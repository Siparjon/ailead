...
def dedupe(leads):
    ...
    for x in leads:
        name = norm(x.get("business_name"))
        domain = norm((x.get("website") or "").split("//")[-1].split("/")[0].removeprefix("www."))
...
def first_line(x):
    name = x.get("business_name", "your business")
    pain = (x.get("potential_pain_points") or [""])[0]
    if pain: return f"I came across {name} and noticed a potential area where finance/admin work may be taking time."
    return f"I came across {name} while researching businesses in your market."


def enrich_ui_fields(leads):
    for x in leads:
        signals = competitor_signals(x)
        x["competitor_signals"] = signals
        x["existing_accounting_support"] = bool(signals)
        ch, why = channel(x)
        x["recommended_channel"] = ch
        x["channel_reason"] = why
        x["personalized_first_line"] = first_line(x)
        # These are prioritization scores based only on collected public evidence.
...
def market_score(country, industry, price):
    # Transparent prioritization heuristic, not a factual income/market-size claim.
    language = {"Australia":25,"United Kingdom":24,"Canada":23,"Singapore":25,"United States":24,"New Zealand":22,"UAE":20,"Malaysia":18}
    fit = {"Digital agency":25,"Marketing agency":25,"Consultant":24,"E-commerce":24,"Property management":22,"Cleaning company":21,"Restaurant":18,"Salon":17,"Contractor":20}
    affordability = {"Australia":25,"United States":25,"Singapore":25,"United Kingdom":23,"Canada":23,"UAE":24,"New Zealand":21,"Malaysia":16}
...
with tab_find:
    with st.sidebar:
        ...
        count = st.slider("Businesses to analyze", 5, 50, 15)
        model = st.text_input("Local Ollama model", "qwen3:4b")
        st.divider(); st.subheader("Quality filters")
        ...
        crawl_pages = st.slider("Pages per website", 1, 10, 3, help="3–5 is usually a good speed/quality balance.")
    ...
    if run:
        with st.status("Finding and qualifying businesses...", expanded=True) as status:
            try:
                ...
                places = dedupe(search_places(location, business_type, count * 3 if enrich else count))
                st.write(f"Found {len(places)} candidates after duplicate filtering.")
                if not places: status.update(label="No businesses found", state="error"); st.stop()
                st.write("2/4 — Searching public web results and websites")
                st.write(f"3/4 — Crawling up to {crawl_pages} public pages per candidate")
    ...
    if leads:
        ...
        for i, lead in enumerate([x for x in leads if x.get("score",0) >= min_score], 1):
            ...
            with st.expander(f"{i}. {lead.get('business_name','Unknown')} — {score}/100 {icon} — {lead.get('recommended_channel','Research')}"):
                ...
                with a:
                    ...
                    st.write(f"**Decision-maker evidence:** {'Yes' if lead.get('decision_maker_evidence') else 'Not found'}")
                with b:
                    st.write(f"**Fit:** {lead.get('business_fit_score',0)}/100")
                    st.write(f"**Potential need:** {lead.get('potential_need_score',0)}/100")
                    st.write(f"**Contactability:** {lead.get('contactability_score',0)}/100")
                    st.write(f"**Decision maker:** {lead.get('decision_maker_score',0)}/100")
                with c:
                    st.write(f"**Best channel:** {lead.get('recommended_channel')}")
                    st.write(f"**Why:** {lead.get('channel_reason')}")
                    st.write(f"**Competitor signals:** {', '.join(lead.get('competitor_signals',[])) or 'None detected'}")
                st.write("**Personalized first line:**", lead.get("personalized_first_line", ""))
                st.write("**AI outreach draft:**")
                st.code(lead.get("outreach", ""), language=None)
                ...
                if st.button("Save to CRM", key="save_" + norm(lead.get("business_name"))):
                    key = lead.get("website") or lead.get("business_name")
                    crm[key] = {**lead, "status":"New", "created":str(date.today()), "next_follow_up":str(date.today()+timedelta(days=3)), "notes":""}
                    save_crm(crm); st.success("Saved to CRM")
        ...
        st.info("Set your criteria and click Find leads.")

with tab_crm:
    if not crm:
        st.info("Save leads from Find Leads to start your CRM.")
    else:
        ...
        for key, lead in list(crm.items()):
            with st.expander(f"{lead.get('business_name',key)} — {lead.get('status','New')}"):
                ...
                status = st.selectbox("Status", STATUSES, index=STATUSES.index(current) if current in STATUSES else 0, key="status_"+norm(key))
                default = date.fromisoformat(lead["next_follow_up"]) if lead.get("next_follow_up") else date.today()+timedelta(days=3)
                follow = st.date_input("Next follow-up", value=default, key="follow_"+norm(key))
                notes = st.text_area("Notes", lead.get("notes",""), key="notes_"+norm(key))
                st.write(f"**Best channel:** {lead.get('recommended_channel','Research')}")
                if status in ("Contacted","Replied","Interested") and follow <= date.today(): st.warning("Follow-up is due.")
                if st.button("Update", key="upd_"+norm(key)):
                    lead.update({"status":status,"next_follow_up":str(follow),"notes":notes}); crm[key]=lead; save_crm(crm); st.rerun()

with tab_analytics:
    if not crm:
        st.info("Save leads to see analytics.")
    else:
        df = pd.DataFrame(list(crm.values()))
...
with tab_market:
    ...
    st.caption("This ranks combinations using a transparent heuristic for testing. It does not claim actual market size, wealth, or conversion rates.")
    countries = st.multiselect("Markets", ["Australia","Singapore","United Kingdom","Canada","United States","UAE","New Zealand","Malaysia"], ["Australia","Singapore","United Kingdom","Canada"])
    industries = st.multiselect("Industries", ["Digital agency","Marketing agency","Consultant","E-commerce","Property management","Cleaning company","Restaurant","Salon","Contractor"], ["Digital agency","Consultant","E-commerce"])
    price = st.number_input("Target monthly price (USD)", 50, 1000, 175, 25)
    if countries and industries:
        rows = [{"Country":c,"Industry":i,"Priority score":market_score(c,i,price)} for c in countries for i in industries]
