...
HTTP_HEADERS = {
    "User-Agent": "ailead-free-mvp/1.5 (personal lead research tool; contact: local-user)",
...
CATEGORY_TAGS = {
    ...
    "dentist": [("amenity", "dentist")], "pharmacy": [("amenity", "pharmacy")], "clinic": [("amenity", "clinic")],
}
...
def geocode_location(location: str) -> tuple[float,float,float,float]:
    r = requests.get(NOMINATIM_URL, params={"q":location,"format":"json","limit":1}, headers=HTTP_HEADERS, timeout=12)
...
def _overpass_query(query: str) -> dict[str,Any]:
    ...
    for url in OVERPASS_URLS:
        for method in ("post", "get"):
            try:
                if method == "post":
                    r=requests.post(url,data={"data":query},headers={**HTTP_HEADERS,"Content-Type":"application/x-www-form-urlencoded"},timeout=30)
                else:
                    r=requests.get(url,params={"data":query},headers=HTTP_HEADERS,timeout=30)
                if r.ok:
                    data=r.json()
                    if data.get("elements"): return data
                    last=f"200 empty from {url}"
                else: last=f"{r.status_code} from {url}"
            except requests.RequestException as e: last=str(e)
    return {"elements":[], "_error":last}


def _nominatim_candidates(location: str, business_type: str, limit: int) -> list[dict[str,Any]]:
    queries=[f"{business_type}, {location}", f"{business_type} {location}"]
    out=[]; seen=set()
    for q in queries:
        try:
            r=requests.get(NOMINATIM_URL,params={"q":q,"format":"json","addressdetails":1,"limit":min(50,limit)},headers=HTTP_HEADERS,timeout=15)
            if not r.ok: continue
            for x in r.json():
                name=(x.get("name") or x.get("display_name","").split(",")[0]).strip()
                if not name or x.get("place_id") in seen: continue
                seen.add(x.get("place_id")); lat=x.get("lat"); lon=x.get("lon")
                out.append({"id":f"nom-{x.get('place_id')}","business_name":name,"address":x.get("display_name",""),"type":business_type,"website":"","phone":"","google_maps":f"https://www.google.com/maps/search/?api=1&query={lat},{lon}" if lat and lon else ""})
...
def search_places(location: str,business_type: str,max_results:int=20)->list[dict[str,Any]]:
    # Primary source: Overpass/OpenStreetMap. Fallback: Nominatim search so a temporary
    # Overpass outage or an OSM tag mismatch does not produce a false "no businesses" result.
    ...
    elif any(k in key for k in ("cafe","coffee")): clauses=f' nwr["amenity"~"cafe|restaurant"]({south},{west},{north},{east});'
    elif any(k in key for k in ("agency","consultant","consulting","company","business")):
        clauses=f' nwr["office"]({south},{west},{north},{east});'
    elif any(k in key for k in ("shop","store","retail","ecommerce","e-commerce")):
        clauses=f' nwr["shop"]({south},{west},{north},{east});'
    else:
        # Broad fallback for natural-language business types: named businesses plus common commercial objects.
        safe=re.sub(r"[^A-Za-z0-9 .&_-]","",business_type).strip()
        clauses=f' nwr["name"~"{safe}",i]({south},{west},{north},{east}); nwr["office"]({south},{west},{north},{east});'
    q=f"[out:json][timeout:25];({clauses});out center tags;"
    data=_overpass_query(q); elements=data.get("elements",[]); out=[]; seen=set()
    for e in elements:
    ...
    if out: return out
    return _nominatim_candidates(location,business_type,max_results)
...
def _search_web(query:str,limit:int=8)->list[dict[str,str]]:
    return [{"title":t,"url":u} for t,u in _search_web_cached(query,limit)]
...
def _extract_page(html:str,base:str)->dict[str,Any]:
    ...
    for a in soup.find_all("a",href=True)[:200]:
        u=urljoin(base,a["href"]); links.append(u); d=_domain(u)
        for sd,label in SOCIAL_DOMAINS.items():
            if d==sd or d.endswith("."+sd): socials.setdefault(label,u)
    return {"text":text[:12000],"emails":emails,"phones":phones,"socials":socials,"links":links}
...
def _crawl_site(root:str,max_pages:int=3)->dict[str,Any]:
    ...
    while q and len(visited)<max_pages:
        ...
        try:
            ...
            data=_extract_page(r.text,r.url); emails.update(data["emails"]); phones.update(data["phones"]); socials.update(data["socials"])
            soup=BeautifulSoup(r.text,"html.parser"); pages.append({"title":soup.title.get_text(" ",strip=True) if soup.title else "","url":r.url})
            low=data["text"].lower()
...
def enrich_lead(business:dict[str,Any],location:str,crawl_pages:int=3,deep:bool=False)->dict[str,Any]:
    name=business["business_name"]
    search=_search_web(f'"{name}" "{location}"',5)
    ...
    site=_crawl_site(website,max(1,min(crawl_pages,5))) if website else {}
    dm_search=[]; social_search=[]
    if deep:
        dm_search=_search_web(f'"{name}" "{location}" owner founder director LinkedIn',5)
        social_search=_search_web(f'"{name}" "{location}" Instagram Facebook',5)
    profiles={}
    ...
    business["search_sources"]=[x["url"] for x in search+dm_search+social_search][:20]
    return business
...
def score_lead(b:dict[str,Any],service:str,ideal_customer:str,model:str="qwen3:4b")->dict[str,Any]:
    prompt=f"You are a careful B2B lead qualification assistant. Service: {service}. Ideal customer: {ideal_customer}. PUBLIC DATA ONLY:\n{json.dumps(b,ensure_ascii=False,indent=2)}\nScore 0-100. Never invent names, revenue, employees, pain points or contacts. Return ONLY JSON: score, fit, reasons, suggested_service, outreach, unknowns."
    r=requests.post(OLLAMA_URL,json={"model":model,"prompt":prompt,"stream":False,"format":"json"},timeout=90); r.raise_for_status(); x=json.loads(r.json().get("response","").strip()); x["score"]=max(0,min(100,int(x.get("score",0)))); return x


def qualify_leads(places:list[dict[str,Any]],service:str,ideal_customer:str,model:str="qwen3:4b",enrich:bool=True,location:str="",require_contact:bool=True,require_website:bool=False,crawl_pages:int=3,progress_callback=None,deep:bool=False)->list[dict[str,Any]]:
